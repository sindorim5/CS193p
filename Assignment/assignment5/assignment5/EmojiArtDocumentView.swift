...
struct EmojiArtDocumentView: View {
        ...
        GeometryReader { geometry in
            ZStack {
                Color.white.overlay(
                    ...
                    .gesture(doubleTapToZoom(in: geometry.size).exclusively(before: deselectAllEmojis()))
                if document.backgroundImageFetchStatus == .fetching {
                    ...
                    ForEach(document.emojis) { emoji in
                        Text(emoji.text)
                            ...
                            .border(isSelected(emoji) ? Color.blue : Color.red, width: 3)
                            .scaleEffect(zoomScale)
                            .position(position(for: emoji, in: geometry))
                            .gesture(toggleEmojiSelectedState(emoji))
                    }
                }
            }
            .clipped()
            ...
            .gesture(!selectedEmojis.isEmpty ? panGestureEmoji().simultaneously(with: emojiZoomGesture()) : nil)
        ...
        DragGesture()
            .updating($gestureEmojiPanOffset) { latestDragGestureValue, gestureEmojiPanOffset, _ in
                selectedEmojis.forEach { emoji in
                    document.moveEmoji(emoji, by: latestDragGestureValue.translation / zoomScale - gestureEmojiPanOffset)
                }
    ...
    // MARK: - Select, Deselect Emoji
    
    @State private var selectedEmojis = Set<EmojiArtModel.Emoji>()
    
    private func toggleEmojiSelectedState(_ emoji: EmojiArtModel.Emoji) -> some Gesture {
        TapGesture()
            .onEnded {
    ...
    private func isSelected(_ emoji: EmojiArtModel.Emoji) -> Bool {
        return selectedEmojis.contains { $0.id == emoji.id }
    }
